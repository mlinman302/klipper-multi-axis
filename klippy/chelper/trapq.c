...
// Return the XYZABC coordinates given a time in a move.  Callbacks that
// only read some of the components rely on link-time optimization
// (-flto -fwhole-program) to discard the rest.
inline struct coord
...
{
    struct coord start_pos = { .x=start_pos_x, .y=start_pos_y
                               , .z=start_pos_z, .a=start_pos_a
                               , .b=start_pos_b, .c=start_pos_c };
